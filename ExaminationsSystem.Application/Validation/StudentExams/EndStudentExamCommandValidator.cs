...
{
    public class EndStudentExamCommandValidator : AbstractValidator<EndStudentExamCommand>
    {
        public EndStudentExamCommandValidator(IStudentExamRepository studentExamRepository)
        {
            RuleFor(i => i)
                .CustomAsync(async (command, context, cancellation) =>
                {
                    ...
                    else
                    {
                        ...
                        if (studentExam.EndDate.HasValue)
                            context.AddFailure(ErrorType.StudentExam_Alreday_Ended.GetDescription());
